...
public class JalaliCalendar {
	...
	/**
	 * Calculates the Julian Day number (JG2JD) from Gregorian or Julian calendar
	 * dates. This integer number corresponds to the noon of the date (i.e. 12
	 * hours of Universal Time). The procedure was tested to be good since 1
	 * March, -100100 (of both the calendars) up to a few millions (10**6) years
	 * into the future. The algorithm is based on D.A. Hatcher, Q.Jl.R.Astron.Soc.
	 * 25(1984), 53-55 slightly modified by me (K.M. Borkowski, Post.Astron.
	 * 25(1987), 275-279).
	 * 
	 * @param year
	 *          <code>int</code>
	...
	private int Jal2JD(int jY, int jM, int jD) {
		JalCal(jY);
		int jd = JG2JD(gY, 3, march, 1) + (jM - 1) * 31 - jM / 7 * (jM - 7) + jD
				- 1;
		return jd;
	}
	...
	public long GregorianToMillis(String gregorianDate) {
	...
	public String GregorianToPersian(long timestamp) {
		Calendar calendar = Calendar.getInstance();
	...
	public String PersianToGregorian(String persianDate) {
		int year = Integer.parseInt(persianDate.substring(0, 4));
		int month = Integer.parseInt(persianDate.substring(5, 7));
		int day = Integer.parseInt(persianDate.substring(8, 10));
	...
	public long PersianToMillis(String persianDate) {
		int year = Integer.parseInt(persianDate.substring(0, 4));
		int month = Integer.parseInt(persianDate.substring(5, 7));
		int day = Integer.parseInt(persianDate.substring(8, 10));
